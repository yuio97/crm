        ...
        <table class="layui-table" lay-filter="mylist" lay-size="lg">
            <thead>
                <tr> 
                    ...
                    <th lay-data="{field:'time',align:'center', minWidth:108}">库存编号</th>
                    <th lay-data="{field:'www',align:'center',minWidth:128}">仓库类型编号</th>
                    <th lay-data="{field:'company',align:'center',minWidth:160}">商品名称</th>
                    <th lay-data="{field:'user',align:'center',width:180}">商品说明</th>
                    <th lay-data="{field:'tel',align:'center',width:150}">商品编号</th>
                    <th lay-data="{field:'addr',align:'center',minWidth:120}">商品数量</th> 
                    <th lay-data="{field:'addr1',align:'center',minWidth:120}">商品单位</th> 
                    <th lay-data="{field:'addr3',align:'center',minWidth:110}">商品售格</th>
                    <th lay-data="{field:'addr4',align:'center',minWidth:110}">备注</th>  
                    <th lay-data="{field:'result',align:'center',minWidth:150}">最后修改日期</th> 
                </tr> 
            </thead>
            <tbody id="kcl">
                <tr v-for="kcgoods in kc" > 
                    
                    <td></td>
                    <td > <a>修改</a>丨<a href="kcgoods_add.jsp">添加</a>丨<a href="javascript:;" style="color:#205b9e;" onclick="Vip_xq()">详情</a> </td>
                    <td>{{kcgoods.kcgoodsId}}</td>
                    <td>{{kcgoods.warehouseId}}</td>
                    <td>{{kcgoods.kgoodsName}}</td>
                    <td>{{kcgoods.kgoodsSm}}</td>
                    <td>{{kcgoods.kgoodsId}}</td>
                    <td>{{kcgoods.kcNum}}</td>
                    <td>{{kcgoods.danwei}}</td>
                    <td>{{kcgoods.lsj}}</td>
                    <td>{{kcgoods.kcMassage}}</td>
                    <td>{{kcgoods.kgoodsLastDate}}</td>  
                </tr> 
       ...
       this_a=this;
        $.ajax({
            ...
            success: function (response) {
            	this_a.kc=response.kcgoodsList;
            	console.log(response.kcgoodsList);
            }
        });
    }
